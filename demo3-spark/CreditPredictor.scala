...
object CreditPredictor extends Serializable {
  ...
  def predict(inputFile: String) {
    ...
    val creditDF = parseRDD(sc.textFile(inputFile)).map(parseCredit).toDF().cache()
...
def inputFile = "/var/lib/spark/data/germancredit.csv"
CreditPredictor.predict(inputFile)

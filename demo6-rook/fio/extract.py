...
def print_clat(name, job_type, job):
    dict = job[job_type]['clat_ns']['percentile']
...
def print_base(name, job_type, job):
    print('{0}, {1}, {2}, {3}, {4}, {5}'.format(
        ...
        job[job_type]['iops'],
        job[job_type]['bw'],
        job[job_type]['lat_ns']['mean'],
        job[job_type]['clat_ns']['percentile']['90.000000'],
        job[job_type]['lat_ns']['stddev'],
        job[job_type]['slat_ns']['mean']
    ))
...
for job in results['jobs']:
    ...
    if 'rand-write-16k' == name or 'rand-read-16k' == name:
        title = name
        for k in sorted(job[job_type]['clat_ns']['percentile'].keys()):
            title += ',{}'.format(k)
        print(title)
